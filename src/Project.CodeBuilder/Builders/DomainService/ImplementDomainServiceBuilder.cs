...
namespace Project.CodeGenerator
{
    internal class ImplementDomainServiceBuilder
    {
        ...
        {
            builder.AppendLine($"        public IQueryable<{entityName}> GetAll().AsNoTracking()");
            builder.AppendLine( "        {");
            builder.AppendLine($"            return _{paramName}Repository.GetAll().AsNoTracking();");
            builder.AppendLine( "        }");
        }

        private void GenerateGetAllAsync(StringBuilder builder,string entityName,string paramName)
        {
            builder.AppendLine($"        public IQueryable<{entityName}> Filter(string keyword)");
            builder.AppendLine( "        {");
            builder.AppendLine("           if (string.IsNullOrWhiteSpace(keyword))");
            builder.AppendLine($"               return _{paramName}Repository.GetAll().AsNoTracking();");
            builder.AppendLine("");
            builder.AppendLine($"            return _{paramName}Repository.GetAll().AsNoTracking().Where(x=>x.Name.Contains(keyword));");
            builder.AppendLine( "        }");
